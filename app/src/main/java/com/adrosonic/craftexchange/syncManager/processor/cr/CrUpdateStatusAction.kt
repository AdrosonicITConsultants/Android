...
import com.adrosonic.craftexchange.services.cr.CrAcceptRejectService
...
class CrUpdateStatusAction : ChangeProcessor<ItemType> {
    ...
    override val predicateForLocallyTrackedElements: String
        get() = "${Orders.COLUMN_ACTION_MARK_CR_STATUS_UPDATE}=1"

    fun actionCr(itemType: ItemType, context: Context){
        var work = Intent().apply { putExtra(CrAcceptRejectService.KEY_ID,itemType.id) }
        CrAcceptRejectService.enqueueWork(context, work)
    }
