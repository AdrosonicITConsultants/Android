package com.adrosonic.craftexchangemarketing.ui.modules.buyer.ownDesign
...
import com.adrosonic.craftexchangemarketing.database.predicates.RelateProductPredicates
import com.adrosonic.craftexchangemarketing.database.predicates.WeaveTypesPredicates
import com.adrosonic.craftexchangemarketing.databinding.ActivityBuyerAddOwnProductDesignBinding
import com.adrosonic.craftexchangemarketing.repository.data.request.artisan.productTemplate.RelatedProduct
import com.adrosonic.craftexchangemarketing.repository.data.request.buyer.OwnDesignRequest
import com.adrosonic.craftexchangemarketing.repository.data.request.buyer.UpdateOwnDesignRequest
...
//            child_step7.visibility = View.VISIBLE
    ...
    fun callUpdate(width:String,length:String){
        ...
        var weavelist=ArrayList<com.adrosonic.craftexchangemarketing.repository.data.request.buyer.ProductWeaf>()
        weaveIdList.forEach { weavelist.add(com.adrosonic.craftexchangemarketing.repository.data.request.buyer.ProductWeaf(System.currentTimeMillis(),productId,it)) }

        var relProdList=ArrayList<com.adrosonic.craftexchangemarketing.repository.data.request.buyer.RelProduct>()
        if (arrRelatedProdType!!.size > 0) {
            var relprod= com.adrosonic.craftexchangemarketing.repository.data.request.buyer.RelProduct(arrRelatedProdType?.get(0)?.id ?: 0,mBinding?.spSubProdWidth?.selectedItem.toString(),mBinding?.spSubProdLength?.selectedItem.toString())
