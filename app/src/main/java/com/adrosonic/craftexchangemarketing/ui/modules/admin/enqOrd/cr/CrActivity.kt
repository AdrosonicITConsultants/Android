...
import com.adrosonic.craftexchangemarketing.database.predicates.CrPredicates
import com.adrosonic.craftexchangemarketing.database.predicates.OrdersPredicates
import com.adrosonic.craftexchangemarketing.databinding.ActivityCrBinding
...
//import com.adrosonic.craftexchangemarketing.ui.modules.order.cr.adapter.CrAcceptRejectAdapter
//import com.adrosonic.craftexchangemarketing.ui.modules.order.revisePi.revisePiContext
...
class CrActivity : AppCompatActivity(),
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        var crStages =
            gson.fromJson(UserConfig.shared.crStatusData.toString(), CrOptionsResponse::class.java)
        stageList.clear()
        stageList.addAll(crStages.data)
        Log.d("CRview", "onCreate: " + stageList)
    ...
    fun setDetails(){
        ...
        when(changeRequestStatus) {
            0L -> {
                    ...
                    mBinding?.txtCountAcceptReject?.text = "CR response pending from Artisan"
//
//
//
...
//                            }
            ...
            1L, 3L, 2L -> {
                mBinding?.txtCountAcceptReject?.text = "Response from Artisan"
...
//                    mBinding?.txtSubmit?.visibility=View.GONE
                var changeReq = CrPredicates.getCrs(enquiryId)
                Log.d("CRview", "setDetails: "+ changeReq)
                var count = 0
                changeReq?.forEach {
                    if (it.requestStatus!!.equals(0L) || it.requestStatus!!.equals(2L)) crSelctionList.add(
                        Pair(it, false)
                    )
                    else {
                        count++
                ...
                mBinding?.txtCountAcceptReject?.text =
                    "Artisan has accepted $count out of ${crSelctionList.size} requests"

            }
                4L->{
                    mBinding?.txtCountAcceptReject?.text = "Something Went Wrong"
