...
import com.adrosonic.craftexchangemarketing.databinding.FragmentRedirectEnqProductDetailsBinding
...
import kotlinx.android.synthetic.main.fragment_common_user.*
...
class RedirectEnqProductDetailsFragment : Fragment(),
RedirectedEnquiryViewModel.ProductDetailsInterface{
     var mBinding : FragmentRedirectEnqProductDetailsBinding?= null
    ...
    override fun onCreateView(
        ...
        mBinding = DataBindingUtil.inflate(inflater, R.layout.fragment_redirect_enq_product_details, container, false)
        arguments?.let {
            productID = it.getLong(ARG_PARAM1)
            roleId = it.getLong(ARG_PARAM2)
            enquiryId = it.getLong(ARG_PARAM3)
        }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        mBinding?.btnRedirectEnq?.setOnClickListener {
                context?.startActivity(context?.selectLessThan8ArtisanActivityIntent(enquiryId?:0))
        }

                when(roleId){
                    RedirectEnqTypes.CUSTOM.getId()->{
                        Log.e("ProductDetails","CUSTOM : $productDetails")
                        mBinding?.btnRedirectEnq?.visibility=View.VISIBLE
                    }
                    RedirectEnqTypes.FAULTY.getId()->{
                        Log.e("ProductDetails","FAULTY : $productDetails")
                        mBinding?.btnRedirectEnq?.visibility=View.VISIBLE
                    }
                    RedirectEnqTypes.OTHERS.getId()->{
                        Log.e("ProductDetails","OTHERS : $productDetails")
                        mBinding?.btnRedirectEnq?.visibility=View.GONE
                    }
                }
    ...
    private fun callApi() {
        viewLoader()
        Log.e("ProductDetails","productID: $productID")
        Log.e("ProductDetails","roleId: $roleId")
        when (roleId) {
            RedirectEnqTypes.CUSTOM.getId()->mViewModel.getCustomProduct(productID?:0)
            RedirectEnqTypes.FAULTY.getId()->mViewModel.getCustomProduct(productID?:0)
            RedirectEnqTypes.OTHERS.getId()->mViewModel.getArtisanProduct(productID?:0)
        }
    }
    fun viewLoader(){
...
//                Utility.displayMessage("Unable to fetch product details",requireContext())
    ...
    override fun onSuccess() {
        try {
            Handler(Looper.getMainLooper()).post(Runnable {
                Log.e("ProductDetails", "onSuccess")
            ...
            Log.e("ProductDetails", "Exception onFailure " + e.message)
        }
    }
    ...
    fun getProductCares(productId : Long?){
        ...
        if (list != null) {
            ...
            if(list.size==0){
                mBinding?.washCareText?.visibility=View.GONE
            }
        }
    ...
    companion object {
        ...
        fun newInstance(param1: Long, param2: Long,param3:Long) =
            RedirectEnqProductDetailsFragment().apply {
