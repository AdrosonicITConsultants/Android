...
import com.adrosonic.craftexchangemarketing.repository.ImageDownloadRepository
...
class ChatListViewModel (application: Application) : AndroidViewModel(application){
    ...
    interface EscalationListInterface{
        fun onGetEscalationListSuccess(list :List<EscSumData>)
    ...
    fun sendChatboxMessage( messageJson : String){
        var token = "Bearer ${Prefs.getString(ConstantsDirectory.ACC_TOKEN,"")}"
        craftexchangemarketingRepository.getChatService()
        ...
        craftexchangemarketingRepository
            ...
            .getEscalationSummary(token,enquiryId).enqueue(object : Callback, retrofit2.Callback<EscalationSummResponse> {
                override fun onFailure(call: Call<EscalationSummResponse>, t: Throwable) {
                    ...
                    Log.d("escalation", "onCreate: escalation failed")
...
//                        ChatUserPredicates?.insertEscalation(response?.body()!!)
                        if(response?.body()!!.data.size>0){
                            escalationListListener?.onGetEscalationListSuccess(response?.body()!!.data)
                        }
                        Log.d("escalation", "onCreate: escalation success")

                    }else
                    {
                        escalationListListener?.onGetEscalationListFailure()
                        Log.d("escalation", "onCreate: escalation invalid")
