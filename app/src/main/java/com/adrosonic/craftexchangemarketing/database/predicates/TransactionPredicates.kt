...
import com.adrosonic.craftexchangemarketing.database.entities.realmEntities.Enquiries
...
import com.adrosonic.craftexchangemarketing.repository.data.response.transaction.CompletedTransactionResponse
import com.adrosonic.craftexchangemarketing.repository.data.response.transaction.OngoingTransactionResponse
...
class TransactionPredicates {
    ...
    companion object {
        ...
        fun insertSingleOngoingTransaction(transacDetails: List<OngoingTransactionResponse?>?){
            val realm = CXRealmManager.getRealmInstance()
            var tranItr = transacDetails?.iterator()
            realm.executeTransaction {
                try {
                    if (tranItr != null) {
                        while (tranItr.hasNext()) {
                            var transac = tranItr.next()
                            var tranObj = realm.where(Transactions::class.java)
                                .equalTo( Transactions.COLUMN_TRANSACTION_ID, transac?.transactionOngoing?.id)
                                .limit(1)
                                .findFirst()
                            ...
                            if (tranObj == null) {
                                ...
                                nextID = tranObj?._id ?: 0
                                Log.e("Transaction","insertSingleTransaction 1111111111111")
                                tranObj?.transactionID = transac?.transactionOngoing?.id
                                tranObj?.enquiryID = transac?.transactionOngoing?.enquiryId
                                tranObj?.enquiryCode = transac?.enquiryCode
                                tranObj?.orderCode = transac?.orderCode
                                ...
                                tranObj?.totalAmount = transac?.transactionOngoing?.totalAmount?:0

                                tranObj?.upcomingStatus =transac?.transactionOngoing?.upcomingStatus
                                tranObj?.accomplishedStatus =  transac?.transactionOngoing?.accomplishedStatus
                                tranObj?.isActionCompleted = transac?.transactionOngoing?.isActionCompleted
                                tranObj?.isActive = transac?.transactionOngoing?.isActive

                                tranObj?.transactionOn = transac?.transactionOngoing?.transactionOn
                                tranObj?.modifiedOn = transac?.transactionOngoing?.modifiedOn
                                tranObj?.completedOn = transac?.transactionOngoing?.completedOn
                                tranObj?.isCompleted = false
                                realm.copyToRealmOrUpdate(tranObj)
                            }
                        }
                    }
        ...
        fun insertSingleCompletedTransaction(transacDetails: List<CompletedTransactionResponse?>?){
            val realm = CXRealmManager.getRealmInstance()
            var tranItr = transacDetails?.iterator()
            realm.executeTransaction {
                try {
                    if (tranItr != null) {
                        while (tranItr.hasNext()) {
                            ...
                            if (tranObj == null) {
                                ...
                                tranObj?.percentage = transac?.transactionCompleted?.percentage
                                tranObj?.paidAmount = transac?.transactionCompleted?.paidAmount
                                tranObj?.totalAmount = transac?.transactionCompleted?.totalAmount?:0
